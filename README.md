...
- Merge multiple policy documents into one
  Define custom policies locally by utilising the `additional_policy_documents` variable and `source_policy_documents` datasource to merge policy documents together and append all policies into one file. This will allow you to work around the kms restriction of only having 1 policy document attached to every key.
...
| <a name="input_additional_policy_documents"></a> [additional_policy_documents](#input_additional_policy_documents) | Additional custom policy documents that can be merged with the module's defined policies to allow flexibility for custom policies. All policy documents must contain unique SIDs for merge to be successful | `list(string)` | `[]`    |    no    |
